package com.lostad.app.common.tag;
...
public class MenuTag extends TagSupport {
	...
	private static String getChildOfTree(Menu parent, int level, List<Menu> menuList) {
		...
		if (level > 0) {//level 为0是功能菜单
			...
			ServletContext context = SpringUtil
		...
		@JsonIgnore
		public static boolean hasPermisson(Menu menuTarget){
			List<Menu> menuList = UserUtils.getMenuList();
			for(Menu menu:menuList){
				if(menu.getId().equals(menuTarget.getId())){
					return true;
				}
			}
			return false;
